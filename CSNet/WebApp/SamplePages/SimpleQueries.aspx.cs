...
using Northwind.Data;// points to our CBOs
#endregion
...
namespace WebApp.SamplePages
{
    public partial class SimpleQueries : System.Web.UI.Page
    {
        ...
        protected void Submit_Click(object sender, EventArgs e)
        {
            ...
            //  or/and this could be via logic code
            int searcharg = 0;
            if (string.IsNullOrEmpty(SearchArg.Text))
            {
                //message to user missing productID for search
                MessageLabel.Text = "Search is missing a Product ID value.";
            }
            else if (int.TryParse(SearchArg.Text, out searcharg))
            {
                //create an instance of the controller class we wish to use
                ...
                //test the returned value for null (not valid)
                if (results == null)
                {
                    //not found: not found message
                    MessageLabel.Text = "No product was found with that ID";
                }
                else
                {
